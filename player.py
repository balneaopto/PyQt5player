#!/usr/bin/env python3
"""
PyQt5 mp3 player. Browse and play your mp3 files by cover artworks and titles in a scroll area. Basic controls.
By balneaopto. Watch video at YouTube channel:
https://www.youtube.com/results?search_query=balneaopto
"""
...
class FormWidget(QtWidgets.QWidget): 
      def __init__(self,parent): 
          ...
          fullmp3dir = homedir+mp3dir     # With expansion tilde is expanded to "/home/username/dir_name/"
          ...
          for position, title in zip(positions, self.f):
              ...
              self.qp = self.qp.scaled(150, 150, Qt.KeepAspectRatio, Qt.FastTransformation)
          ...
          self.playbutton.setStyleSheet("QPushButton { image-align: right; background-color: #F5F6F7; color: white; border: none}")
          ...
          self.show()
          self.player = subprocess.Popen(["mplayer"]) 
      ...
      def on_button_clicked(self, button_or_id):  
          ...
          if self.flag01 == True:
              ...
              # A song is already playing. Stopping mplayer with q option.
                  if fi == None:  
                      self.player.stdin.write("q".encode('utf-8'))
      ...
      def skipbuttonHandler(self):
          ...
          if (self.track+1) == self.multipleofeight:  
              self.k = self.multipleofeight + 1
          # If the button at the bottom of the page, scroll the scroll area
      ...
      def play(self,mediafile):
          self.player = subprocess.Popen(["mplayer",mediafile," -cache 1024"," -cache-min 10", " -ao alsa:device=hw=0.0"],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
